...
class ExperimentVisualizer:
    ...
    def plot_radar_chart(self, baseline_results: Dict, lhns_results: Dict):
        ...
        if 'TSP50' in baseline_results:
            for algo_name, result in baseline_results['TSP50']['algorithms'].items():
                all_qualities.append(result['avg_distance'])
        ...
        if lhns_results:
            for method, result in lhns_results.items():
                all_qualities.append(result.get('best_obj', 999))
                all_times.append(result.get('time', 100))
        ...
        # 传统算法数据（归一化到0-1，值越小质量越好，所以反转）
        traditional_values = [
            (max_quality - trad_quality) / quality_range if quality_range > 0 else 0.5,
            (max_time - trad_time) / time_range if time_range > 0 else 0.5,
            0.8,   # 稳定性
            0.3    # 创新性
        ]
        ...
        # LHNS数据
        lhns_values = [
            (max_quality - lhns_quality) / quality_range if quality_range > 0 else 0.5,
            (max_time - lhns_time) / time_range if time_range > 0 else 0.5,
            0.6,   # 稳定性
            0.95   # 创新性
        ]
