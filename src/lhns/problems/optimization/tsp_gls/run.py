...
from LHNS.problems.optimization.tsp_gls.utils import readTSPRandom
from LHNS.problems.optimization.tsp_gls.gls.gls_run import solve_instance
from func_timeout import func_set_timeout, FunctionTimedOut

class TSPGLS():
    def __init__(self, problem_type='white-box') -> None:
        self.n_inst_eva = 64 # a samll value for test only
    ...
    def evaluateGLS(self, heuristic):
        ...
        inputs = [(x, self.opt_costs[x], self.instances[x], self.coords[x], self.time_limit, self.ite_max,
                   self.perturbation_moves, heuristic) for x in range(nins)]
        # gaps = Parallel(n_jobs=nins)(delayed(solve_instance)(*input) for input in inputs)
        try:
            gaps = Parallel(n_jobs=4, timeout=self.time_limit * 1.1)(
    ...
    def evaluate(self, code_string, task='train', scale=20):
        path = os.path.dirname(os.path.abspath(__file__))
        if task == 'train':
            self.instance_path = path + '/TrainingData/TSPAEL64.pkl'
        elif task == 'test':
            self.instance_path = path + f'/TestingData/TSP{scale}.pkl'
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                #print(code_string)
                fitness = self.evaluateGLS(code_string)
